...
class TestRunTask:
    ...
    def test_passes_extra_args(self, tmp_path):
        runner = FakeSubprocessRunner().set_popen(["BUILD SUCCESSFUL\n"], 0)
        GradleRunner(runner, tmp_path, platform="linux").run_task("testDebugUnitTest", extra_args=["-DtestType=unit"])
        assert runner.called_with("testDebugUnitTest", "-DtestType=unit")
    ...
    def test_streams_output_lines_to_log(self, tmp_path, capsys):
        runner = FakeSubprocessRunner().set_popen(["> Task :app:assembleDebug\n", "BUILD SUCCESSFUL\n"], 0)
        GradleRunner(runner, tmp_path).run_task("assembleDebug")
        captured = capsys.readouterr()
        assert "> Task :app:assembleDebug" in captured.out
