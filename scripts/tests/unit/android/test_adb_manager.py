...
from tests.helpers.adb_fixtures import (
    ADB_DEVICES_EMPTY,
...
class TestWaitForEmulator:
    ...
    def test_boot_loop_polls_boot_completed_without_getprop_w(self):
        # getprop -w (wait-for-change) isn't supported by every emulator image's
        # toybox getprop — confirmed live against a real AVD, where it printed
        # "invalid option -- w" in a tight busy-loop instead of blocking. The
        # portable poll-with-sleep must not regress back to -w.
        runner = (
            FakeSubprocessRunner()
            .add_run(returncode=0)
            .add_run(returncode=0)
        ...
        assert "getprop -w" not in shell_script
        assert "sleep" in shell_script
