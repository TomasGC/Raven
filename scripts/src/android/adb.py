        ...
        try:
            ...
            result = self._runner.run(
                [
                    ...
                    emu,
                    "shell",
                    # `getprop -w` (wait-for-change) isn't supported by every emulator
                    # image's toybox getprop (confirmed: "invalid option -- w" on a real
                    # device here, which busy-loops printing errors instead of blocking
                    # until boot_completed flips) — a plain poll-with-sleep is slightly
                    # less instant but portable across images, which matters more.
